...
:rhow:      =  fbar( :rhow: )
:Et:        =  fbar( :Et:   )
# Make sure internal energy and density are conserved
:IE: = :Et:/:rho: - .5*(:u:*:u: + :v:*:v: + :w:*:w:)
:rho: = ibmS(:rho:,:phi:,[:gx:,:gy:,:gz:])
:IE: = ibmS(:IE:,:phi:,[:gx:,:gy:,:gz:])
:Et: = :rho: * ( :IE: + 0.5*(:u:*:u: + :v:*:v: + :w:*:w:) )
# Update the primatives and enforce the EOS
:u:         =  :rhou: / :rho:
:v:         =  :rhov: / :rho:
:w:         =  :rhow: / :rho:
...
:betaT: = 0.01 * gbar( abs( ring( :div:) ) ) / sqrt(:LsA:**2 + :LsB:**2 + :LsC:**2)
# :betaT: = gbar(abs(dd4x(:div:)))*:LsA: + gbar(abs(dd4y(:div:)))*:LsB: + gbar(abs(dd4z(:div:)))*:LsC:
:betaA: = :rho: * :betaT: * :dA:
:betaB: = :rho: * :betaT: * :dB:
:betaC: = :rho: * :betaT: * :dC:
:cs:  = dt.IBsqroot( :p: / :rho: * :gamma: )
